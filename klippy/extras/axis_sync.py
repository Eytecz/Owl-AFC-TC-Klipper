...
class GCodeAxisSync:
    def __init__(self, config):
        ...
        self.gcode.register_command("SYNC_MANUAL_STEPPER", self.cmd_SYNC_MANUAL_STEPPER,
                                    desc="Synchronize manual_stepper with existing axes")
    ...
    def cmd_SYNC_MANUAL_STEPPER(self, gcmd):
        ...
        stepper_object = self.printer.lookup_object(name, None)
        if stepper_object is None:
            raise gcmd.error(f"Invalid manual stepper name '{name}'")

        stepper = stepper_object.get_steppers()[0]
        toolhead = self.printer.lookup_object('toolhead')
        motion_queuing = self.printer.lookup_object('motion_queuing')

        # Always flush before attaching/re-attaching
        toolhead.flush_step_generation()

        # --- UNSYNC CASE ---
        if extruder_name is None or extruder_name.lower() == "none":
            if hasattr(stepper, "_orig_trapq") and stepper._orig_trapq is not None:
                stepper.set_trapq(stepper._orig_trapq)
                stepper._orig_trapq = None
                gcmd.respond_info(f"Manual stepper {name} restored to its original trapq")
            return

        # --- SYNC CASE ---
        extruder = self.printer.lookup_object(extruder_name, None)
        if extruder is None:
            raise gcmd.error(f"Invalid extruder name '{extruder_name}'")

        # Save original trapq
        stepper._orig_trapq = stepper.get_trapq()

        # Synchronize with extruder’s motion
        stepper.set_position([extruder.last_position, 0., 0.])
        stepper.set_trapq(extruder.get_trapq())

        motion_queuing.check_step_generation_scan_windows()
        gcmd.respond_info(f"Manual stepper {name} synced to {extruder_name}")
